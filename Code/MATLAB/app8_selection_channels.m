...
select_channel = 1:64; 
num_removed_channels = 10;
select_channel(removed_channels(1:num_removed_channels))= [];

for i = 1:size(target_data, 3)
    x1 = target_data(:, select_channel, i);
    x2 = non_target_data(:, select_channel, i);
...
labels = [ones(1, size(features1, 2)), -ones(1, size(features2, 2))];
model = fitcsvm(data', labels, 'Standardize', 1, 'BoxConstraint', 0.01);
%% ------- Step 7: Word detection in all runs using the training model training -------
